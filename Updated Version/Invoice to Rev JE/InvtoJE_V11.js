...
define(['N/record', 'N/log'], function (record, log) {
    ...
    function handleInvoice(rec) {
        ...
        log.debug(
            'Customer Invoice',
            'Checking item lines for account 1650'
        );
        ...
        for (var i = 0; i < lineCount; i++) {
            ...
            log.debug(
                'Invoice Line Account ' + i,
                lineAccount
            );

            if (parseInt(lineAccount) === ACCOUNT_1650) {
        ...
        if (creditLines.length === 0) {
            ...
            log.debug(
                'Skipped',
                'Customer Invoice - No item lines found with account 1650'
            );
        ...
        return {
            ...
            memoText:
                'Reversal JE for Customer Invoice ' + rec.id,
    ...
    function handleVendorPayment(rec) {
        ...
        for (var i = 0; i < applyLineCount; i++) {
            ...
            for (var j = 0; j < expenseLineCount; j++) {
                ...
                if (
                    parseInt(expenseAccount) === ACCOUNT_2009
                ) {
            ...
            var appliedAmount = rec.getSublistValue({
                sublistId: 'apply',
                fieldId: 'amount',
                line: i
            });
            ...
            debitTotal += parseFloat(appliedAmount) || 0;

            log.debug(
                'Valid Applied Bill',
                billId + ' Amount: ' + appliedAmount
            );
        }
        ...
        if (creditLines.length === 0) {
            ...
            log.debug(
                'Skipped',
                'Vendor Payment - No applied Vendor Bills found with expense account 2009'
            );
